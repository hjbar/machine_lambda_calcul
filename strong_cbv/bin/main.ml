...
let () =
  ...
  if not @@ alpha_equiv t res then begin
    pp_result t res;
    failwith "Error with test1 in strong_cbv_cps"
  end
  else pp_result t res
...
let () =
  ...
  eval omega |> pp_lambda;
  newline_flush ();
  failwith "Error with test2 in strong_cbv_cps : omega's reduction is finite"
*)
...
let () =
  ...
  if not @@ alpha_equiv cinq res then begin
    pp_result t res;
    failwith "Error with test3 in strong_cbv_cps"
  end
  else pp_result t res

let () = pritnln_flush "strong_cbv_cps : OK"
