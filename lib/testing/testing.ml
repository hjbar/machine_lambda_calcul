...
let test_weak_cbnd_all ~naif_interp ~cps_interp ~defunc_interp ~version_name =
  let version_name = String.capitalize_ascii version_name in

  println_flush @@ version_name ^ " tests :";
  println_newline ();

  let err1 = test_random_weak naif_interp cps_interp "naif" "cps" in
  let err2 = test_random_weak naif_interp defunc_interp "naif" "defunc" in
  let err3 = test_random_weak cps_interp defunc_interp "cps" "defunc" in
