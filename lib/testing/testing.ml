...
let test_gen_interp ~test_interp ~naif_interp ~cps_interp ~defunc_interp ~version_name =
  let version_name = String.capitalize_ascii version_name in
  ...
  let err2 = test_interp naif_interp defunc_interp "naif" "defunc" in
...
let test_weak_interp = test_gen_interp ~test_interp:test_random_weak
...
let test_strong_cbnd_all =
  test_gen_all ~test_ref:test_random_strong_cbn_with_reference ~test_interp:test_random_strong

let test_strong_interp = test_gen_interp ~test_interp:test_random_strong
