...
let test () =
  ...
  let () =
    try
      ...
      (* Fin test *)
      println_flush @@ green_string "Strong_cbv tests : OK";
      print_newline ()
    with err ->
      begin
        ...
        println_flush @@ red_string "Strong_cbv tests : ERROR";
        print_newline ();
        error := true
      end
  in
  ...
  let () =
    try
      ...
      println_flush @@ green_string "Strong_cbv Random tests : OK";
      print_newline ()
    with err ->
      begin
        println_flush @@ Printexc.to_string err;
        ...
        println_flush @@ red_string "Strong_cbv Random tests : ERROR";
        print_newline ();
        error := true
      end
  in

  if !error then println_flush @@ red_string "Strong_cbv : ERROR"
  else println_flush @@ green_string "Strong_cbv : OK"
