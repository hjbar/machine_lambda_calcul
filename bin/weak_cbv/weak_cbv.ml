...
let test () =
  ...
  let () =
    try
      ...
      println_flush "Weak_cbv Random tests : OK";
      print_newline ()
    with err ->
      begin
        println_flush @@ Printexc.to_string err;
        ...
        println_flush "Weak_cbv Random tests : ERROR";
        print_newline ();
        error := true
      end
  in

  if !error then println_flush "Weak_cbv : ERROR"
  else println_flush "Weak_cbv : OK"
