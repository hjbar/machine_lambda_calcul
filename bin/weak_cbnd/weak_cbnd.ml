  ...
  let () =
    ...
    with err ->
      begin
        ...
        println_error "Weak_cbnd tests : ERROR";
        print_newline ();
        error := true
      end
  in
  ...
  let () =
    try
      ...
      *)
      ...
      println_ok "Weak_cbnd Random tests : OK";
      print_newline ()
    with err ->
      begin
        println_flush @@ Printexc.to_string err;
        ...
        println_error "Weak_cbnd Random tests : ERROR";
        print_newline ();
        error := true
      end
  in

  if !error then println_error "Weak_cbnd : ERROR"
  else println_ok "Weak_cbnd : OK"
